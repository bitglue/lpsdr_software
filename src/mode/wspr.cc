...
WSPR::~WSPR() {
  if (not m_recoding_file.empty()) {
    remove(m_recoding_file.c_str());
  }
  if (not m_decoding_file.empty()) {
...
bool WSPR::on_finish_recording() {
  ...
  auto argv = std::list<std::string>{"wsprd", "-w", m_decoding_file.c_str()};
  Glib::spawn_async_with_pipes(
      m_wav_dir, // working directory
      argv, flags,
...
std::string WSPR::filename_for_interval(unsigned interval) const {
  char buf[512];
  snprintf(buf, 512, "%s/lpsdr_wspr_%i.wav", m_wav_dir.c_str(), interval);
  return buf;
}
